...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		// DONE?: Index Movie attributes "title", "rating", "votes",
		// "tweets.coordinates"
	...
	public void saveTweet(String movie, Status status) {
		...
		BasicDBObject tweet = new BasicDBObject().append("user", user).append("text", text).append("retweet", retweet).append("date", date);
		...
		movies.update(new BasicDBObject("title", movie), new BasicDBObject("$push", new BasicDBObject("tweets", tweet)), false, true);
	}
	...
	public DBCursor getTweetsNear(double lat, double lng, int radiusKm) {
		...
		DBObject pointQuery = new BasicDBObject("coordinates", new BasicDBObject("$near", new BasicDBObject("$geometry", new BasicDBObject("type", "Point").append("coordinates", new Double[] { lng, lat })).append("$maxDistance", radiusKm * 1000)));
